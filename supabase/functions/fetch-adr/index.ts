...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    for (const pair of pairsToFetch) {
      try {
        const url = `https://api.twelvedata.com/time_series?symbol=${pair}&interval=1day&outputsize=15&apikey=__API_KEY__`;
        const resp = await fetchWithRotation(url, "twelvedata", sb);
